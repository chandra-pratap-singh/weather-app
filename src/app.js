    ...
    if(query.city)
    {
        getgrid(query.city,(error,data)=>{
            if(error){
                ...
                getforcast(data.longitude, data.lattitude,data.location, (error,data)=>{
                    if(error)
                    {
                        resData = {error:true,message:"Sorry unable to get the forcast"}
                        res.send(resData);
                        return console.log(data);
                    }
                    else{
        ...
        res.send({error:true,message:"please enter a city name!"});
